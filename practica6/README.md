# Enrich authors
In this practice, we enrich the information about a set of authors stored in a local RDF graph using data from WikiData and DBpedia SPARQL endpoints.
There are several queries to perform the enrichment, each one stored in the `queries` folder.
Use [this script](src/main/java/Main.java) to execute all the queries in order.
For each query, the resulting triples are added to the a file with the name `qX.ttl`, where `X` is the number of the query in the `output` folder.
